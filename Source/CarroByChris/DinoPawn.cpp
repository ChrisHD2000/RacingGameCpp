...
ADinoPawn::ADinoPawn() {
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> PersonajeMesh(TEXT("/Game/Assets/MallasPersonajes/RanaStatic/RanaStatic.RanaStatic"));
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> MeshAsset(TEXT("/Game/CarroPartes/CoberturaDino.CoberturaDino"));
...
void ADinoPawn::Tick(float Delta) {
	Super::Tick(Delta);
	
}

void ADinoPawn::BeginPlay() {
	Super::BeginPlay();
	
	

	FTimerHandle tiempo;
	GetWorldTimerManager().SetTimer(tiempo, this, &ADinoPawn::ComenzarCarrera, 4.f);
}

void ADinoPawn::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);
	// Enlazamos las entradas de teclado con sus métodos
	check(PlayerInputComponent);

	PlayerInputComponent->BindAxis("MoveForward", this, &ADinoPawn::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &ADinoPawn::MoveRight);
	PlayerInputComponent->BindAxis("RotateSteeringWheel", this, &ADinoPawn::RotarVolante);
	PlayerInputComponent->BindAction("Handbrake", IE_Pressed, this, &ADinoPawn::OnHandbrakePressed);
	PlayerInputComponent->BindAction("Handbrake", IE_Released, this, &ADinoPawn::OnHandbrakeReleased);
	PlayerInputComponent->BindAction("RetornoVolante", IE_Released, this, &ADinoPawn::RegresoVolante);
	PlayerInputComponent->BindAction("CambiarCamara", IE_Pressed, this, &ADinoPawn::CambiarCamara);
	PlayerInputComponent->BindAction("CambiarCamara", IE_Released, this, &ADinoPawn::RetornarCamara);
	PlayerInputComponent->BindAction("PowerUp", IE_Pressed, this, &ADinoPawn::UsePowerUp);
	PlayerInputComponent->BindAction("Shoot", IE_Pressed, this, &ADinoPawn::Fire);
	
}

void ADinoPawn::MoveForward(float Val)
{
	if (comenzo)
	GetVehicleMovementComponent()->SetThrottleInput(Val);
}
void ADinoPawn::MoveRight(float Val)
{
	
	GetVehicleMovementComponent()->SetSteeringInput(Val);
}
void ADinoPawn::OnHandbrakePressed()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(true);
}

void ADinoPawn::OnHandbrakeReleased()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(false);
}

void ADinoPawn::RotarVolante(float val) {
...
void ADinoPawn::RegresoVolante() {
	volante->SetRelativeRotation(FRotator(pitch, 0.f, roll));
}
void ADinoPawn::CambiarCamara() {
	Camera->Deactivate();
	camTrasera->Activate();
}

void ADinoPawn::RetornarCamara() {
	camTrasera->Deactivate();
	Camera->Activate();
}
void ADinoPawn::ComenzarCarrera()
{
	comenzo = true;
}

void ADinoPawn::UsePowerUp()
{
